...
//    }
    ...
    static func Global_Get<T:NSObject>(
        ...
        Model:((_ model:AppResultModel?) -> Void)?,
        ModelNoAnalysis: ((_ model:Any?) -> Void)?)->Void{
        if(ModelNoAnalysis != nil){
            ...
            AFNHelper.get(nil, urlString: url, parameters:parameters,isHUD: isHUD,HUDMsg:HUDMsg,isHUDMake: isHUDMake, success: { (json) in
                ...
                if(json != nil){    //==nil表示失败的
                    ...
                    if(entity != nil){
                        if(IsListData==true){
                            let model = T.mj_objectArray(withKeyValuesArray: json?.description)
                            value=model
                        }else{
                            let model = T.mj_object(withKeyValues: json?.description)
                            value=model
                        }
                        
                        
                        ModelNoAnalysis?(value)
                    }
                }else{
                    if(ModelNoAnalysis != nil){
                        ModelNoAnalysis?("error")
                    }
                }
